...
class RunnerTest < Test::Unit::TestCase
  context "with a file to test and a destination to verify" do
    ...
    should "be able to run a runner over ssh" do
      ssh = Medusa::SSH.new(
        ...
        "ruby -e \"require 'rubygems'; require 'medusa'; Medusa::Runner.new(:io => Medusa::Stdio.new, :verbose => true);\""
      )
      assert ssh.gets.is_a?(Medusa::Messages::Runner::RequestFile)
      ssh.write(Medusa::Messages::Worker::RunFile.new(:file => test_file))

      # grab its response. This makes us wait for it to finish
      echo = ssh.gets # get the ssh echo
      response = ssh.gets

      assert_equal Medusa::Messages::Runner::Results, response.class

      # tell it to shut down
      ssh.write(Medusa::Messages::Worker::Shutdown.new)
    ...
    context "using runner events" do
      context "on successful termination" do
        ...
        should "fire runner_begin event" do
          run_the_runner(@pipe,  [MedusaExtension::RunnerListener::RunnerBeginTest.new] )
          Process.wait(@parent)
        ...
        should "fire runner_end event" do
          run_the_runner(@pipe,  [MedusaExtension::RunnerListener::RunnerEndTest.new] )
          Process.wait(@parent)
  ...
  module RunnerTestHelper
    def request_a_file_and_verify_completion(pipe, file)
      ...
      assert pipe.gets.is_a?(Medusa::Messages::Runner::RequestFile)
      pipe.write(Medusa::Messages::Worker::RunFile.new(:file => file))
      
      # grab its response. This makes us wait for it to finish
      response = pipe.gets
      
      # tell it to shut down
      pipe.write(Medusa::Messages::Worker::Shutdown.new)
    ...
    def run_the_runner(pipe, listeners = [])
      ...
      Medusa::Runner.new( :io => pipe, :runner_listeners => listeners )
    end
  end
