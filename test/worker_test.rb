...
class WorkerTest < Test::Unit::TestCase
  ...
  module WorkerTestHelper
    def run_the_worker(pipe, num_runners)
      ...
      Medusa::Worker.new({:io => pipe, :runners => num_runners})
    end
    ...
    def request_a_file_and_verify_completion(pipe, num_runners)
      ...
      num_runners.times do
        ...
        assert response.is_a?(Medusa::Messages::Worker::RequestFile), "Expected RequestFile but got #{response.class.to_s}"
      end
      pipe.write(Medusa::Messages::Master::RunFile.new(:file => test_file))

      assert pipe.gets.is_a?(Medusa::Messages::Worker::Results)
