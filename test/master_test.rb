...
class MasterTest < Test::Unit::TestCase
  context "with a file to test and a destination to verify" do
    ...
    should "run even with a test that will not require" do
      class FileOutputListener < Medusa::Listener::Abstract
    ...
    should "run a spec with pending examples" do
      progress_bar = Medusa::Listener::ProgressBar.new(StringIO.new)
      Medusa::Master.new(
    ...
    should "synchronize a test file over ssh with rsync" do
      local = File.join(Dir.consistent_tmpdir, 'medusa', 'local')
      remote = File.join(Dir.consistent_tmpdir, 'medusa', 'remote')
      sync_test = File.join(File.dirname(__FILE__), 'fixtures', 'sync_test.rb')
  ...
  context "with a runner_end event" do
    setup do
      ...
      @runner_listener = 'MedusaExtension::RunnerListener::RunnerEndTest.new' # runner_end method that creates alternate_target_file
      @master_listener = MedusaExtension::Listener::WorkerBeganFlag.new  #used to know when the runner is up
    end
  ...
  context "redirecting runner's output and errors" do
    ...
    should "create the default runner log file when passing an incorrect log file path" do
      default_log_file = "#{remote_dir_path}/#{Medusa::Runner::DEFAULT_LOG_FILE}" # medusa-runner.log"
  ...
  def copy_worker_init_file
    @remote_init_file = "#{remote_dir_path}/#{File.basename( medusa_worker_init_file )}"
    ...
    # copy the medusa_worker_init to the correct location
    FileUtils.cp(medusa_worker_init_file, remote_dir_path)
  end
end
